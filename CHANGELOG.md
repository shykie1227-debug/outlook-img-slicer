...
## 未发布（post-V6.3.0）- 2026-09-23

### Outlook 几何与链接
- 热区邮件排版由「连续外表格 + 每个视觉行嵌套单行表」改为**单表统一列网格**：
  取所有视觉行 X 边界的并集作为全局列定义，用 `<colgroup>` 承载列宽、用 `colspan`
  让每行覆盖自己跨越的列，标记为 `data-layout="hotspot-grid"`。
  消除经典 Outlook 在嵌套表边界重算行高所产生的可见缝隙。
- `_build_cell()` 新增 `colspan` 支持。
- 新增热区行宽一致性硬校验：某行宽度之和 != 邮件显示宽度时直接抛
  `ValueError`，不再静默产出错位 HTML。

### 质量保障
- 热区排版回归测试同步为统一列网格断言：嵌套行表数 5 → 0，`<tr>` 数 11 → 6，
  并校验 `<colgroup>` 宽度求和等于显示宽度、每行 `colspan` 之和等于列数。
- 全量测试 166 passed；CI 同款 `compileall` 校验通过。
