...
def test_v640_version_is_synchronized_everywhere():
    version_info = (DESKTOP_ROOT / "version_info.txt").read_text(encoding="utf-8")
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    spec = (ROOT / "SPEC.md").read_text(encoding="utf-8")

    assert main.VERSION == "6.4.2"
    assert "6.4.2.20260924" in version_info
    ...
    assert components == [6, 4, 2, 2026]
    assert all(0 <= value <= 65535 for value in components)
    assert "OutlookImgSlicer-V6.4.2.exe" in readme
    assert "V6.4.2" in spec
