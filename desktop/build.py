#!/usr/bin/env python3
"""
desktop/build.py - Outlook 长图助手桌面版一键打包脚本
用法: python build.py
...
def pause_before_exit():
    ...
    if os.environ.get("OUTLOOK_IMG_SLICER_NO_PAUSE") == "1":
        return
...
def create_onedir_fallback(dist_dir: Path = None, build_dir: Path = None):
    ...
    cmd = [
        ...
        f"--add-data={PROJECT_ROOT / 'icon.ico'}{os.pathsep}.",
        f"--add-data={PROJECT_ROOT / 'icons'}{os.pathsep}icons",
        "main.py",
    ]
    if dist_dir is not None:
...
def main():
    print("=== Outlook 长图助手桌面版打包脚本 ===")
    print(f"项目目录: {PROJECT_ROOT}")
    print(f"桌面应用目录: {APP_DIR}")
    ...
    spec_file = APP_DIR / "outlook_img_slicer.spec"
